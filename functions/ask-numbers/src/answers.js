...
export async function answerRandomIntent(event, emit, handler, attributes, storage) {
  ...
  const randomNumber = randomArray[randomIndex][0];
  if (parseInt(answer, 10) === randomNumber) {
...
export async function answerInorderIntent(event, emit, handler, attributes, storage) {
  ...
  const inorderNumber = Numbers[inorderIndex][0];
  // True answer
  if (parseInt(answer, 10) === inorderNumber) {
...
async function randomTrueAnswer(event, emit, handler, attributes, storage) {
  ...
  let smallimageurl = randomArray[randomIndex][1];
  let largeimageurl = randomArray[randomIndex][2];
  /* eslint-disable */
  ...
  if (typeof randomArray[randomIndex] !== 'undefined') {
    ...
    smallimageurl = randomArray[randomIndex][1];
    largeimageurl = randomArray[randomIndex][2];
  }
...
async function inorderTrueAnswer(event, emit, handler, attributes, storage) {
  ...
  let totalscore = attributes.dataToSave.totalscore;
  let numerate = attributes.dataToSave.numerate;
  let inorderNumber = Numbers[inorderIndex][0];
  let smallimageurl = Numbers[inorderIndex][1];
  let largeimageurl = Numbers[inorderIndex][2];
  /* eslint-disable */
  attributes.dataToSave = {
    randomArray: randomArray, inorderArray: Numbers, randomIndex: randomIndex, inorderIndex: inorderIndex, numerate: numerate, score: score, totalscore: totalscore,
  };
  ...
  if (typeof Numbers[inorderIndex] !== 'undefined') {
    inorderNumber = Numbers[inorderIndex][0];
    smallimageurl = Numbers[inorderIndex][1];
    largeimageurl = Numbers[inorderIndex][2];
  }
  // Answer is true but no more number
  if (typeof Numbers[inorderIndex] === 'undefined') {
    const oldNumber = -1;
...
async function inorderFalseAnswer(event, emit, handler, attributes, storage) {
  ...
  if (parseInt(answer, 10) !== inorderNumber && numerate < 2) {
    ...
    inorderIndex += 1;
    numerate = 0;
    const oldNumber = Numbers[inorderIndex - 1][0];
    if (typeof Numbers[inorderIndex] !== 'undefined') {
      inorderNumber = Numbers[inorderIndex][0];
      smallimageurl = Numbers[inorderIndex][1];
      largeimageurl = Numbers[inorderIndex][2];
...
function gameOver(emit, handler, oldNumber, totalscore) {
  ...
  emit(':askWithCard', speechOutput, reprompt, 'Game is over', totalscore.toString(), cardImage);
}
...
function getTotalScore(totalscore) {
  return Math.round((totalscore / 33) * 100);
}
